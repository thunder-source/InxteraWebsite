...
export default function Home() {
  ...
  return (
    <>
      ...
      <section
        className='w-full flex justify-center items-center flex-col gap-4 mt-48 md:mt-72 mb-16 px-4'
        id='top'>
        ...
        <Link href='/#download' className='md:hidden'>
          <Button>{t('more.download')}</Button>
        </Link>
        <Phone
          data={phone.elements.map((item) => ({
            ...item,
      ...
      <section
        ...
        {moreFeatures.map(({ img, id, note, icon }, i) => {
          const Icon: ComponentType<IconProps> =
            icon.startsWith('@') && icons.hasOwnProperty(icon.slice(1))
              ? (icons as any)[icon.slice(1)]
              : () => unsafelyLoadSVG(localisePath(icon));
          return (
            <figure
              className={
                'flex w-full justify-between gap-8 md:gap-24 items-center flex-col-reverse ' +
              ...
              />
              <figcaption className='flex flex-col justify-center gap-1 text-center md:text-left md:w-7/12 h-fit'>
                {Icon && (
                  <div
      ...
      <div
        ...
        <Marquee
          rotation={8}
          baseVelocity={-1}
          className='text-white text-outline-neutral-300'
          scrollBoost={0.25}>
          {splashes.join(' ')}
        </Marquee>
      </div>
