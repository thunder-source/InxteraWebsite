...
export default function Cursor({ smoothnessCoefficient = 0.9 }: CursorProps) {
  ...
  const trailSize = useRef<{ width: number; height: number }>({
    width: 0,
    height: 0,
  });
  ...
  useEffect(() => {
    ...
    const move = () => {
      hoveredElement.current = document.elementFromPoint(
        mousePosition.current.x,
        mousePosition.current.y
      );
      const customTextParent =
        hoveredElement.current?.closest('[data-cursor-text]');
      if (trail.current && pullText.current) {
        const computedStyle = hoveredElement.current
          ? getComputedStyle(hoveredElement.current)
          : null;
        const cursorStyle = computedStyle?.cursor || 'default';
        const parentBoundingRect = customTextParent?.getBoundingClientRect();
        const maxParentDimension = Math.max(
          parentBoundingRect?.width || 0,
        ...
        trailSize.current.width =
          trailSize.current.width * smoothnessCoefficient +
          trailSizeTarget.width * (1 - smoothnessCoefficient);
        trailSize.current.height =
          trailSize.current.height * smoothnessCoefficient +
          trailSizeTarget.height * (1 - smoothnessCoefficient);
        ...
        pullOpacity.current =
          pullOpacityTarget < 0.001
            ? 0
            : pullOpacity.current * smoothnessCoefficient +
              pullOpacityTarget * (1 - smoothnessCoefficient);
    ...
    <div className='fixed top-0 left-0 w-full h-full pointer-events-none z-50 cannot-hover:hidden'>
      <div
        className={`absolute transform -translate-x-1/2 -translate-y-1/2 rounded-full pointer-events-none
                ${
                  light ? 'bg-primary-100' : 'bg-primary-500'
        ...
        <div
          ...
          <span
            className={`text-sm font-bold ${
              light ? 'text-primary-500' : 'text-primary-100'
            } transition-colors duration-300 ease-in-out`}>
            {text}
          </span>
        </div>
